...
ResultOutputPath="$VeinsExamplePath/results/phase2"
...
function RunVeinsCode(){
    ...
    AhpCsvFile="applications_ahp_$numberVehicles.csv"
    RlCsvFile="applications_rl_$numberVehicles.csv"
    FzCsvFile="applications_fz_$numberVehicles.csv"
    tempString="_$SimulationOrderNumber.csv"


    NewAhpCsvFile="applications_ahp_$numberVehicles$tempString"
    NewRlCsvFile="applications_rl_$numberVehicles$tempString"
    NewFzCsvFile="applications_fz_$numberVehicles$tempString"

    if test -f "$AhpCsvFile"; then
        mv $AhpCsvFile $NewAhpCsvFile
    fi
    if test -f "$RlCsvFile"; then
        mv $RlCsvFile $NewRlCsvFile
    fi
    if test -f "$FzCsvFile"; then
        mv $FzCsvFile $NewFzCsvFile
    fi
}
